...
- `--sweep` runs the same experiment at every worker count instead of one, because a routing
  strategy that looks fine with eight deciders and collapses at forty has not been measured by a
  single run.
- The four strategy columns are the comparison: `first`, `best`, `random` and `two`. `best` is the
  greedy choice part [2.3](2.3-with-perfect-information-greedy-is-fine.md) argued for, and `two` is
  the sampled pair this part's paper is about.
- `dropped` is separate from `spilled` on purpose: a spilled request found a second lane, a dropped
  one found none, and only the second is a request the desk failed to serve.
