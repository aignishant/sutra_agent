...
- The docstring opens with what the class does and then states `Layer:` explicitly. A reader deciding
  whether to move this code finds the previous decision before they make a new one.
- It names the **consequence** of under-coverage — the router reads low and sheds load it did not need
  to — rather than asserting that the rule is important. A justification a reader can check is one
  they can also disagree with on evidence, which is the point of writing it down at all.
- *"that is a bug, not a choice"* is [6.1](../06-failure-lab/6.1-the-rule-that-broke-an-agent-nobody-was-looking-at.md)'s
  question answered in the file where the answer has to hold, in the words the question was asked in.
- The last sentence gives the second, independent reason: the run-level doors
  ([2.2](../02-the-fourteen-doors/2.2-the-four-doors-an-agent-cannot-have.md)) do not exist on an
  agent. Even if coverage were a choice, this rule could not be a callback.
...
- The same `Layer:` line, and this time it records a **negative**: not a plugin, deliberately. The
  cheapest thing to lose is the reasoning behind something that was not done, because the code carries
  no trace of it.
- The docstring names the **specific agent** that would break. `6.1`'s failure took a
