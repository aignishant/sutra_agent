...
- `NOTES` is a `Path` pointing at the directory the desk is allowed to read.
- `/` is `pathlib`'s join operator, and it is doing exactly what a path library should: it
  applies the argument as a path expression rather than treating it as a filename.
- The whole expression is one line and it is the vulnerability. Nothing here is a mistake —
  there is no missing check to add at this line, because the line is correct.
