...
- `gate.py` is run from inside `lab/`, like every script in this day, because they import their
  siblings by plain name.
- `echo "exit: $?"` prints the exit code, because a gate is a test rather than a report — something
  is supposed to run it and act on the answer.
