...
- `rows` is retrieved text: written by customers, by insiders, or by whatever a tool talked to.
- The f-string interpolates it into the **system instruction**, which is the one span the model
  is trained to weight most heavily — so untrusted text has been promoted to the most
  authoritative position in the prompt by a formatting convenience.
- Nothing here is a security control being bypassed. There was never one to bypass: the decision
  is made by which string the rows are concatenated into.
...
def no_retrieved_text_in_instruction(callback_context, llm_request):
    ...
    for ref, text in retrieved_rows(callback_context):
        ...
        if probe and probe in instruction:
            return LlmResponse(
                content=say(f"Refused: {ref} was interpolated into the instruction.")
            )
    return None
```
