...
| playwright | 1.62.0 | 2026-09-06 | 71 | Browser automation behind Day 71's `BaseComputer`. Looked up on PyPI before pinning (`pypi.org/pypi/playwright/json`: `info.version` 1.62.0, `requires_python` >=3.10). ADK's own computer-use sample pins 1.52.0; current was installed and the version recorded here rather than copying the sample's pin (Principle 7). **Needs a second, non-pip step:** `uv run playwright install chromium` downloads the browser (~115 MiB) to `~/AppData/Local/ms-playwright/`, which `uv sync` alone does **not** do — a checkout with the package and without the browser fails at `chromium.launch()`. Verified on install day by driving a real headless Chromium against the day's local fixture. |
